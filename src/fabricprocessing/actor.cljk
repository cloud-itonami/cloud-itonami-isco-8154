(ns fabricprocessing.actor
  "FabricProcessingOperationsActor — the ISCO-08 8154 community fabricprocessing actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one fabricprocessing operation request
  ...
  The unconditional invariant: the FabricProcessingOperationsAdvisor can never
  directly commit a record the FabricProcessingOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [fabricprocessing.advisor :as advisor]
            [fabricprocessing.governor :as governor]
            [fabricprocessing.store :as store]))

(defn build-graph
  "Build a compiled FabricProcessingOperationsActor graph. `store` implements
  `fabricprocessing.store/Store`. `advisor` implements
  `fabricprocessing.advisor/Advisor` (defaults to `mock-advisor`).
